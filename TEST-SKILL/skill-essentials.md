...
14. **Foto annunci (luglio 2026 — agg. agosto 2026)** — servite da `righettoimmobiliare.it/img/immobili/` (GitHub Pages), **solo WebP**. Dopo upload in admin: sync **automatico** ogni 6 h via `.github/workflows/sync-media-github.yml` — **PC spento non blocca**. Verifica gate: `python scripts/verify_media_migration.py` (0 Supabase in DB, 0 non-WebP). Dettaglio: **`TEST-SKILL/skill-media-migration.md`**
...
17. **Efficienza sito (agosto 2026)** — **`TEST-SKILL/skill-efficienza-sito.md`**: diff minimo, gate `verify_media_migration.py` + `verify_ga_consent_live.py`, GA4 unico `G-PHEL8KXLBX`, no filler blog. Venerdì: checklist §4 skill-efficienza.
18. **Comando editoriale fisso blog (agosto 2026 — PERMANENTE)** — **`TEST-SKILL/skill-editoriale-visivo.md`**: ricerca strategica §8–17 + visivo §1–7 + §16-TER continuità + §16-QUATER/QUINQUIES varietà visiva/FAQ/proprietari + §18 pubblicabilità; gate `audit_editorial_research.py` + `audit_editorial_continuity.py` + `audit_editorial_visual_variety.py` + `audit_blog_visuals.py` + `audit_blog_publishability.py`. Memorie: `editorial-memory.json` + `editorial-visual-memory.json`. Precedenza su selezione argomento e composizione visiva.
17. **Trasparenza AI Act UE (PRIORITÀ PERMANENTE)** — su **ogni** pagina pubblica, foto, chatbot e contenuto: barra `site-ai-disclosure`, etichetta assistente automatizzato, note foto annunci, link `privacy#trasparenza-digitale`. Dettaglio completo: **`TEST-SKILL/skill-ai-act-compliance.md`** e **`SKILL-2.0.md` §8.1f**
