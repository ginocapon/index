# -*- coding: utf-8 -*-
"""Registra articolo casa vendibile 5 anni in blog.html, admin, homepage, sitemap."""
...
REG = json.loads((ROOT / "scripts" / "casa_vendibile_5anni_registry.json").read_text(encoding="utf-8"))
MARKER = "blog-casa-vendibile-5-anni-case-green-padova-2026"
...
def main() -> None:
    ...
    if MARKER in text:
        ...
        print(f"blog.html: +{len(blog_entries)}")
    ...
    if MARKER in atext:
        ...
        print(f"admin.html: +{len(admin_entries)}")
    ...
    if hp_path.is_file():
        ...
        if MARKER in htext:
            ...
            print(f"homepage.js: +{len(hp_entries)}")
    ...
    for e in blog_entries:
        ...
        url = (
            ...
            f"<lastmod>{e['data']}</lastmod><changefreq>monthly</changefreq><priority>0.7</priority></url>\n"
        )
        sm = sm.replace("</urlset>\n", url + "</urlset>\n", 1)
        added += 1
    if added:
        sm_path.write_text(sm, encoding="utf-8")
        print(f"sitemap.xml: +{added}")
    else:
